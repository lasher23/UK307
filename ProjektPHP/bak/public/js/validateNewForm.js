			...
			.on(
					...
					function() {
						...
						if ($('#email').val() === '') {
							addError($('#email'));
							$('#errorEmail').html(
									'Das Feld E-Mail darf nicht leer sein!');
							errors++;
						} else if (!(new RegExp(".{0,}(@).{0,}").test($(
								'#email').val()))) {
							addError($('#email'));
							$('#errorEmail')
									.html(
											'Das Feld E-Mail muss ein @ Zeichen enthalten');
							errors++;
						}
						...
						if (!(new RegExp("^[0-9\\+\\-)(\\s]*$")
							...
							$('#errorPhone')
									.html(
											'Das Feld Telefon darf nur folgende Zeichen enthalten: +-()1234567890');
						}
						console.log(errors);
						if (errors > 0) {
							return false;
